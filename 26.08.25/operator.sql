...
select * from worker where first_name = 'Varun' or last_name = 'Kumar';
...
select first_name, joining_date, department, salary from worker where (joining_date < '2019-01-01' or salary > 400000) 
and not department = 'Admin';
...
select first_name, department, salary from worker where department in ('admin', 'HR') and not salary = 30000;
...
select first_name, department, joining_date, salary from worker where department not in ('admin') and joining_date < '2014-09-01'
and salary > 60000;
...
select first_name, department, salary, joining_date from worker where department in ('HR', 'Admin') and salary between 50000 and 90000
and joining_date between '2014-01-01' and '2014-12-31';
select first_name, department, salary, joining_date from worker where salary between 25000 and 35000 
or (department = 'Admin' and joining_date between '2014-06-01' and '2014-12-31');
...
select avg(salary) as high_earners_avg from worker where salary > 100000;
...
end as salary_category from worker;
